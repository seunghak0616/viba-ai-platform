/**
 * Gemini AI Service
 * Google Gemini API를 사용한 자연어 처리 서비스
 */
...
class GeminiService {
  constructor() {
    if (!config.gemini.apiKey) {
      logger.warn('Gemini API 키가 설정되지 않았습니다. 모의 응답을 사용합니다.');
      this.mockMode = true;
      this.genAI = null;
      this.model = null;
    } else {
  ...
  async convertToBIMParameters(naturalLanguageInput, language = 'ko') {
    try {
      ...
      // JSON 응답 파싱
      try {
        const bimParameters = JSON.parse(text);
        return this._validateAndNormalizeBIMParameters(bimParameters);
      } catch (parseError) {
        logger.error('Gemini 응답 JSON 파싱 실패', {
...
"This building is a [characteristic] [building type] with [main spaces] efficiently arranged in a total area of [area]. [Special advantages or features] make it optimized for [intended use]."`
  ...
  _getMockBIMParameters(input) {
    // 기존 analyzeNaturalLanguage 함수 결과를 Gemini 형식으로 변환
